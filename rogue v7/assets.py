...
# 상태이상 한글 이름이랑 설명, combat.py _status_display에서 가져다 씀
# 새 상태이상 추가하면 combat.py의 process_turn_statuses랑 _monster_turn_logic에도 처리 로직 추가하면끝
STATUS_INFO = {
...
# 직업 기본 스펙, player.py에서 get_class()로 deepcopy해서 캐릭터 만듦
# v6에서 마법사 HP 50에서 60으로 올림, 광역기 한 방에 죽는 경우가 너무 많았음
CLASSES = {
    "전사": {
...
# 새 카드 추가할 때 class는 "전사"/"마법사"/"공용" 중 하나
# 공용 카드는 전투 보상에 안 뜨고 이벤트로만 얻을 수 있음 (v5에서 바꾼 거)
CARDS = {
    # 전사 전용
...
MONSTERS = {
    ...
    "늑대": {
        "name": "늑대", "stage": 1,
        "base_hp": 50, "base_atk": 11, "base_spd": 14,
    ...
    "오크광전사": {
        ...
        "pattern": ["power", "normal", "normal", "power"],
        "reward_gold": 22,
    ...
    "저주기사": {
        ...
        "pattern": ["vulnerable_strike", "normal", "power"],
        "reward_gold": 40,
    ...
    "광분전사": {
        ...
        "pattern": ["normal", "power", "quick", "aoe"],
        "reward_gold": 50,
...
# 보스는 층수 고정 (15층, 30층)
# v6에서 심연의 근원 패턴 3턴에서 4턴으로 늘림, 광역기가 너무 자주 나옴
BOSSES = {
    15: {
        "name": "숲의 군주", "base_hp": 300, "base_atk": 22, "base_spd": 10,
...
# 유물 데이터
# bonus_energy, poison_start, hp_drain, duration_bonus, burn_start, dodge_chance 같은 키는
# 전투 중 매 턴 계산하는 방식이라 player.py _apply_relic_stats에서 즉시 반영 안 함
RELICS = {
    # 1스테이지 유물
...
# 유물 합성 조합, 두 유물 동시에 보유하면 player.py의 _check_synthesis에서 자동 합성
# 키는 name 필드 기준으로 써주기
RELIC_SYNTHESIS = {
    # 1스테이지 합성
...
# 랜덤 사건 목록, effect 키는 world.py _apply_effect랑 맞춰야 함
# chance 키 있으면 그 확률로만 효과 적용, 없으면 100%
RANDOM_EVENTS = [
    {
        "name": "오래된 샘터",
...
def get_monster(name, floor):
    # deepcopy 안 하면 MONSTERS 원본이 바뀜, 같은 몬스터 두 번 만나면 수치가 이상해지는버그 때매고침
    # 스케일 공식: 1 + 층수 x FLOOR_SCALE
    base  = copy.deepcopy(MONSTERS[name])
    scale = 1 + (floor * FLOOR_SCALE)
    base["hp"]  = int(base["base_hp"]  * scale)
    base["atk"] = int(base["base_atk"] * scale)
    base["spd"] = base["base_spd"]  # SPD는 고정
    return base

def get_boss(floor):
    # 보스 스케일은 일반 몬스터의 절반으로 적용, 안 그러면 너무 강해짐
    # SPD도 스케일링 없음
...
def get_stage_relics(stage, count=3, exclude=None):
    # exclude로 이미 보유/구매한 유물 걸러냄, 합성으로 소모된 것도 포함
