...
SPRITE_DIR = "sprites"
SPRITE_SIZE = (64, 32)  # 적 스프라이트 (너비:높이 = 2:1 → 정방형 이미지가 정상 비율로 보임)
PARTY_SPRITE_SIZE = (48, 24)  # 아군 스프라이트 (너비:높이 = 2:1 → 정방형 이미지가 정상 비율로 보임)
try:
...
def _sprite_basename(name: str):
    # 맵핑이 있으면 사용, 없으면 가능한 ASCII 변환을 시도
    if name in SPRITE_KEY_MAP:
        return SPRITE_KEY_MAP[name]
    # 기본: 공백을 언더바로 바꾸고 소문자화
    try:
        n = name.replace(' ', '_')
    except Exception:
        n = name
...
def _render_sprite(name, size=None):
    # 1순위: sprites/{name}.png
    # 2순위: sprites/error.png  (캐릭터 전용 이미지 없을 때)
    # 3순위: 텍스트 (rich-pixels 미설치 시)
    if size is None:
        size = SPRITE_SIZE
    if not PIXELS_AVAILABLE:
        return Text("  [rich-pixels 미설치]\n", style="dim red")
    # support animated frames: sprites/{name}_0.png, {name}_1.png, ...
    frames = []
    # try multiple candidate base names to tolerate single/double underscore mismatches
    candidates = [name]
    if '_' in name:
    ...
    # try single-file candidates: {name}.png → {name}_def.png(기본 스프라이트) → error.png
...
def _select_frame(frames, cycle_seconds: float = 0.7):
    """Return the appropriate frame from a list based on current time.

    cycle_seconds: total seconds for a full cycle through all frames.
    """
    if not frames:
        return None
    if len(frames) == 1:
        return frames[0]
    # compute normalized position in cycle [0.0, 1.0)
...
def draw_combat_screen(player, monster, turn, intent=None, message=None):
    ...
    # 적 몬스터는 스프라이트 1장뿐 - action_frame 없이 기본 스프라이트 사용
    ...
    inner.add_column(ratio=1, min_width=24)
    inner.add_column(ratio=1, min_width=24)
    inner.add_row(_char_section(back, False), _char_section(front, True))

    party_panel = Panel(
        inner,
        title="[bold bright_green]PARTY[/bold bright_green]",
    ...
    grid.add_column(ratio=1)
    grid.add_row(party_panel, enemy_panel)
    console.print(grid)
...
def _handle_card_use(player, monster, active, turn):
    ...
    try:
        ...
        # show action frame briefly
        # 딜 있는 카드 or 마법사 독(독구름) → attack
        # 딜 없는 디버프(weak/vulnerable/burn/stun) + 힐/버프 계열 → buff
...
def _execute_card_effects(card, active, player, monster):
    ...
    if card.get('damage_mult', 0) > 0:
        ...
        if card['target'] == 'enemy_all':
            ...
            _log(f"[bold red]💥 광역 폭발! {monster['name']}에게 {dmg} 피해![/bold red]")
        else:
            monster['current_hp'] -= dmg
...
def _try_dodge(player, active):
    # 바람의 날개 등 dodge_chance 유물 보유 시 단일 공격 회피 판정
    # AOE 공격에는 호출하지 않음
    dodge = sum(r.get('effect', {}).get('dodge_chance', 0) for r in player.relics)
