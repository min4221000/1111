...
def handle_combat(player, floor):
    ...
    try:
        if floor in [15, 30]:
            ai_manager.get_boss_narrative(monster['name'], stage)
        else:
            ai_manager.get_battle_intro(monster['name'], stage)
    except Exception as e:
        console.print(f"[bold yellow]내레이션 생성 중 오류: {e} — 계속 진행합니다.[/bold yellow]")
    time.sleep(0.5)

    # 덱이 비어 있으면 전투가 바로 터지니까 미리 체크
...
def handle_shop(player, floor):
    ...
    try:
        ai_manager.get_shop_narrative(stage)
    except Exception as e:
    ...
    while True:
        try:
            ...
            if 0 <= idx < len(event['choices']):
                ...
                try:
                    ai_manager.get_event_narrative(
...
def _apply_effect(player, effect):
    # elif 대신 if를 연속으로 쓰는 이유:
    # 하나의 effect 딕셔너리에 여러 키가 동시에 있을 수 있어서, elif 쓰면 두 번째 효과가 무시됨
    # 예: {"party_hp": -10, "give_card": "마나 회복"}
    # 새 effect 키 추가하면 assets.RANDOM_EVENTS 이벤트에서 그 키를 써야 함

    if 'gold' in effect:
        # 골드는 chance 무관하게 항상 지급
