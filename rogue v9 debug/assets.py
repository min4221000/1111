...
# 새 상태이상 추가하면 combat.py의 process_turn_statuses랑 _monster_turn_logic에도 처리 로직 추가하면 끝
STATUS_INFO = {
...
# 직업 기본 스펙, player.py에서 get_class()로 deepcopy해서 캐릭터 만듬
# 마법사 HP 50에서 60으로 올림-> 광역기 한 방에 죽는 경우가 너무 많았음
CLASSES = {
    "전사": {
...
# 카드 데이터
# 새 카드 추가할 때 class는 "전사"/"마법사"/"공용" 중 하나로
# 공용 카드는 전투 보상에 안 뜨고 이벤트로만 얻을 수 있게
CARDS = {
    # 전사 전용
...
# 몬스터 데이터, base_hp/atk/spd는 스케일 전 원본 수치임
# 실제 전투 수치는 get_monster()가 층수 반영해서 스케일링 
MONSTERS = {
    # 1스테이지
    "고블린": {
        "name": "고블린", "stage": 1,
...
BOSSES = {
    ...
    30: {
        ...
        "reward_gold": 500,
        "speech": "너희의 여정은 여기서 끝난다. 나는 모든 것을 삼킨다."
    },
}

# 유물 데이터
# 전투 중 매 턴 계산하는 방식이라 player.py _apply_relic_stats에서 즉시 반영은 안함
RELICS = {
    # 1스테이지 유물
...
# 유물 합성 조합, 두 유물 동시에 보유하면 player.py의 _check_synthesis에서 자동 합성되게 ㄱㄱ
# 키는 name 필드 기준으로 써주기
RELIC_SYNTHESIS = {
    # 1스테이지 합성
...
def get_monster(name, floor):
    # deepcopy 안 하면 몬스터 원본 바뀜. 같은 몬스터 두 번 만나면 수치가 이상해지는 버그 있는거고침
...
def get_boss(floor):
    # 보스 스케일은 일반 몬스터의 절반으로 적용함, 안 그러면 너무 강해짐
    # SPD도 스케일링 없음
...
def get_stage_relics(stage, count=3, exclude=None):
    # exclude로 이미 보유/구매한 유물 걸러내게함, 합성으로 소모된 것도 포함
